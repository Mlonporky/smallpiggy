...
3. WASD／方向键移动；E／空格阅读和调查，也可靠近后点击红纸或小树枝。
4. 苏醒处左下方有一根小树枝，醒来后随时可捡；捡起时小呆猪说“还好有一个小树枝。”“至少现在我不会太害怕了……”。巫师离开且已拿到树枝后，向画面上方走进森林；没拿树枝时北路不开。山洞里J／Z攻击，移动躲开史莱姆蓄力冲撞。失败后自动回山洞入口，保留树枝。
5. 击败史莱姆时自动保存一次（此时退出，下次碎片仍在地上，不必重打）；拾取红色碎片后再自动保存。Esc 打开暂停菜单，整个场景（包括史莱姆）停住，可选「继续探索」或「保存并回主菜单」，再按 Esc 继续。此版本完成至第一块碎片，尚未制作山庄汇合与结局。

## 场景与复用

- `scenes/chapter_02_dark_forest/forest_clearing.tscn` 保留原路由地址，现承载新入口。原战斗样板完整保存在同目录 `legacy_forest_sample.tscn`，旧脚本不改。
- `scenes/chapter2/entrance.gd`：已被传送后的苏醒、拾小树枝、红纸、巫师、两段残影、Joy、后退提示、正式迈步、标题与切场。
- `scenes/chapter2/cave.tscn` / `cave.gd`：战斗区域、史莱姆、碎片、失败重试。进入山洞时已持树枝；从调试菜单直接进山洞或旧存档没有树枝标记时，自动补上。
...
- 没有捡起动作：捡小树枝时直接换成持棍行走表。
- 可选的入口停留提示未做；山庄汇合、后续碎片、结局未纳入本轮。

## 验证

Godot 4.7.2。`tests/chapter2_test.gd`覆盖入口输入锁、真实道路阻挡、完整对白推进、巫师离场、重复调查、状态序列化恢复、拾树枝前北路阻挡与拾取台词、真实出口触发与SceneRouter切场、真实HitBox三击战胜史莱姆、碎片去重、已完成状态恢复与失败重试。
