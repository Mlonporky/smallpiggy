...
func tick(delta: float) -> void:
 ...
 if weapon:
  ...
  hitbox.position = Vector2((20+weapon.reach/2)*attack_direction,-45)
 if state=="ACTIVE":
  for enemy in hitbox.get_overlapping_bodies():
   ...
   var query := PhysicsRayQueryParameters2D.create(body.global_position+Vector2(0,-45),enemy.global_position+Vector2(0,-28),1)
 ...
 queue_redraw()
func _draw() -> void:
 if state!="ACTIVE" or not weapon: return
 draw_set_transform(Vector2(0,-45),0,Vector2(attack_direction,1))
 draw_arc(Vector2.ZERO,weapon.reach,-0.8,0.8,16,Color(0.85,0.93,0.75,0.8),5)
