...
- 小呆猪：林间小路与发光洞口、100 点生命、空格跳跃、40% 生命触发“想起白菜”、快乐挥击、史莱姆前摇与扑击、拾取第一块礼物碎片。
...
按键：`WASD` / 方向键移动，`E` 调查（第一幕也支持空格），第二幕自由移动时空格跳跃、对白时空格继续，`J` / `Z` 攻击，第二幕 `Shift` 翻滚、`K` 树枝招架（减伤75%），`F5` 快速保存。
